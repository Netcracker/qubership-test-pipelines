...
def main(args_):
    ...
    # Delete existed ClusterRoles and ClusterRoleBindings and SCC
    cluster_roles_files = None
    cluster_roles_binding_files = None
    scc_files = None
    if cloud_type == 'openshift':
        if os.path.exists('./restricted/clusterRoles/openshift/'):
            print(f'\033[36m----------- Clean UP Cluster Resources from cluster -----------\033[0m')
            cluster_roles_files = file_lib.get_files('./restricted/clusterRoles/openshift/')
        if os.path.exists('./restricted/scc/'):
            scc_files = file_lib.get_files('./restricted/scc/')
            prepare_scc(k8slib, scc_files, namespace)
            if scc_files:
                scc_names = list(file_lib.get_resources_name(scc_files))
                if args_.service:
                    scc_names = [item for item in scc_names if args_.service in item]
                print(f'\033[36m Following SCC should be deleted in cloud: {scc_names} \033[0m')
                k8slib.delete_scc_by_name(scc_names, namespace)
    else:
        if os.path.exists('./restricted/clusterRoles/kubernetes/'):
            print(f'\033[36m----------- Clean UP Cluster Resources from cluster -----------\033[0m')
            cluster_roles_files = file_lib.get_files('./restricted/clusterRoles/kubernetes/')
    ...
    print(f'\033[32m----------- Existed ClusterRoles, ClusterRoleBindings and SCC are deleted! -----------\033[0m')

    # Create ClusterRoles and ClusterRoleBindings and SCC
    if cluster_roles_files:
        if args_.service:
    ...
    if cloud_type == 'openshift':
        if scc_files:
            if args_.service:
                scc_files = [item for item in scc_files if args_.service in item]
            print(f'\033[36m Start to create the following SCC: {scc_files} \033[0m')
            k8slib.create_security_context_constraints_by_path(scc_files)
...
def prepare_scc(k8slib, scc_files, namespace):
    for scc in scc_files:
        with open (scc) as file:
            file_content = yaml.safe_load(file)
            namespace_line = file_content.get('users')
            x = namespace_line[0].split(":")
            x[2] = namespace
            users = ':'.join(x)
            file_content['users'][0] = users
        with open (scc, "w") as file:
