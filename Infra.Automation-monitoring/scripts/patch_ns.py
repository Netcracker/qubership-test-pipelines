...
def main(args_):
    ...
    if args_.label == 'add':
        k8slib.annotate_namespace_labels(args_.namespace, 'privileged')
        print(f'Label privileged successfully added to ns: {args_.namespace}')
    elif args_.label == 'remove':
        k8slib.remove_namespace_label(args_.namespace)
        print(f'Label privileged successfully removed from ns: {args_.namespace}')
